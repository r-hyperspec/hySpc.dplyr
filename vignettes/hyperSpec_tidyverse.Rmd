...
## Mutate hyperSpec data columns
`mutate()` mutates the data columns of `hyperspec` objects. Unlike `transmute()` `mutate()`adds new variables and keeps existing ones.
...
chondro %>%
  mutate (y, x) %>% 
  setLabels(y = expression("x"), x = expression("y"))
...
## Transmute hyperSpec data columns
`transmute()` mutates the data columns of `hyperspec` objects. Unlike `mutate()` `transmute()`adds and only keeps new variables.
